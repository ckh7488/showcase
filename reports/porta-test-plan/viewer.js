...
(() => {
  ...
  try {
    ...
    const motorLabel=label(boards.Motor_board,'Motor / M12',-3,-27,4.7,'#c5e7d6',16);motorLabel.visible=false;
    ...
    const planMarkers=[];
    for(const s of ['cable','phy'])for(const pair of ['rx','tx'])for(const role of ['source','probe']){
      const p=PCB_PLAN_POINTS[s][pair][role],pos=xy(p.board,p.xy),parent=boards[p.board],color=role==='source'?0xedaa65:0x66d9c0;
      const marker=new THREE.Mesh(new THREE.SphereGeometry(.6,14,10),new THREE.MeshBasicMaterial({color,depthTest:false}));
      marker.position.set(...pos,2.6);marker.renderOrder=10;parent.add(marker);
      const stem=new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(...pos,1.534),new THREE.Vector3(...pos,2.6)]),new THREE.LineBasicMaterial({color,depthTest:false}));parent.add(stem);
      const caption=document.createElement('button');caption.textContent=(role==='source'?'① 입력 / ':'② 관측 / ')+p.ref+' '+pair.toUpperCase();caption.dataset.role=role;caption.onclick=()=>focusMarker(role);document.getElementById('pcb-plan-labels').append(caption);
      planMarkers.push({segment:s,pair,role,marker,stem,caption,data:p});
    }
    function updatePlanMarkers(){for(const m of planMarkers){const show=m.segment===segment&&m.pair===(pairFocus==='both'?'rx':pairFocus);m.marker.visible=m.stem.visible=show;m.caption.hidden=!show;}}
    function positionPlanLabels(){
      const w=el.clientWidth,h=el.clientHeight,placed=[];
      for(const m of planMarkers){if(!m.marker.visible){m.caption.hidden=true;continue;}const p=m.marker.getWorldPosition(new THREE.Vector3()).project(camera);m.caption.hidden=p.z< -1||p.z>1;if(m.caption.hidden)continue;const ew=m.caption.offsetWidth,eh=m.caption.offsetHeight;let x=Math.max(6,Math.min(w-ew-6,(p.x+1)*w/2-ew/2)),y=Math.max(44,Math.min(h-eh-6,(1-p.y)*h/2-eh-8));for(let i=0;i<8&&placed.some(r=>x<r.x+r.w+5&&x+ew+5>r.x&&y<r.y+r.h+4&&y+eh+4>r.y);i++)y=Math.min(h-eh-6,y+eh+5);m.caption.style.left=x+'px';m.caption.style.top=y+'px';placed.push({x,y,w:ew,h:eh});}
    }
    function focusMarker(role){
      if(role==='all'){setView('rx');return;}
      if(role==='reference'){document.getElementById('inner-toggle').checked=true;for(const g of copperGroups)g.visible=true;setView('all');return;}
      root.updateMatrixWorld(true);const m=planMarkers.find(m=>m.role===role&&m.marker.visible);if(!m)return;
      const center=m.marker.getWorldPosition(new THREE.Vector3()),box=new THREE.Box3().setFromCenterAndSize(center,new THREE.Vector3(16,14,9));frameBox(box,new THREE.Vector3(8,-20,22),1.3);
      window.dispatchEvent(new CustomEvent('planner:point',{detail:{role,text:(role==='source'?'입력 후보: ':'관측 후보: ')+m.data.board+' / '+m.data.ref+' / '+m.data.pads.map(p=>p.pin+'번 '+p.net).join(', ')+'. 실제 패드 좌표이며 포트의 크기·참조 도체는 해석 전에 별도로 정의합니다.'}}));
    }
    ...
    function setPair(next,notify=true){
      ...
      updatePlanMarkers();setGround();setView('rx');
    ...
    function setSegment(next,notify=true){
      ...
      controlLabel.visible=false;molexLabel.visible=next==='cable';for(const l of phyLabels)l.visible=false;
      ...
      updatePlanMarkers();setGround();setView(next==='phy'?'all':'rx');
      if(notify&&changed)emitSelection();
    }
    ...
    el.addEventListener('keydown',e=>{
      if(e.key==='Home'){e.preventDefault();setView(mode);return;}
      const s=new THREE.Spherical().setFromVector3(camera.position.clone().sub(controls.target));
      if(e.key==='ArrowLeft')s.theta-=.12;else if(e.key==='ArrowRight')s.theta+=.12;
      else if(e.key==='ArrowUp')s.phi=Math.max(.15,s.phi-.12);else if(e.key==='ArrowDown')s.phi=Math.min(Math.PI-.15,s.phi+.12);
      else if(['+','='].includes(e.key))s.radius*=.88;else if(e.key==='-')s.radius*=1.12;else return;
      e.preventDefault();camera.position.copy(controls.target).add(new THREE.Vector3().setFromSpherical(s));controls.update();
    });
    function resize(){if(!el.clientWidth||!el.clientHeight)return;renderer.setSize(el.clientWidth,el.clientHeight);camera.aspect=el.clientWidth/el.clientHeight;camera.updateProjectionMatrix();setView(mode);}
    ...
    function frame(){requestAnimationFrame(frame);if(inView){controls.update();renderer.render(scene,camera);positionPlanLabels();}}
    frame();message.hidden=true;message.style.display='none';
    window.__pcbViewer={ready:true,focusMarker,select:({segment:nextSegment,pair})=>{
      let changed=false;
      ...
      planMarkers:planMarkers.filter(m=>m.marker.visible).map(m=>({role:m.role,pair:m.pair,...m.data})),markerStatus:'pad-location explanation, not solver ports'})};
