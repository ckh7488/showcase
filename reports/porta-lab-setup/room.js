(() => {
  ...
  try{
    ...
    const selectable=[],labels=[];let view='room',explode=0;
    ...
    function resize(){renderer.setSize(el.clientWidth,el.clientHeight);camera.aspect=el.clientWidth/el.clientHeight;camera.updateProjectionMatrix();setView(view);}new ResizeObserver(resize).observe(el);controls.addEventListener('change',draw);
