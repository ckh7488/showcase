"""TEST 01 openEMS 120 mm four-pair topology screening.
...
Existing TEST 01 results remain untouched; this runner writes new candidate
directories and records the role placement and power-pair polarity explicitly.
"""
import argparse
import hashlib
...
p.add_argument('--topology',choices=['adjacent','diagonal'],default='adjacent',
               help='Pair-role placement in the square four-pair bundle.')
p.add_argument('--power-polarity',choices=['aligned','opposed'],default='aligned',
               help='For mixed +/0 pairs, reverse the conductor assignment in the second power pair when opposed.')
...
args=p.parse_args()
if args.wiring=='same' and (args.topology!='adjacent' or args.power_polarity!='aligned'):
    p.error('The legacy +/+ and 0/0 baseline is defined only for adjacent/aligned placement.')
...
for filename in ['run_topology_screening.py','check_pec_connections.py']:
...
layouts={
    'adjacent': {'ethernet':[0,1], 'power':[2,3]},
    'diagonal': {'ethernet':[1,2], 'power':[0,3]},
}
layout=layouts[args.topology]
role_by_slot={slot:('ethernet',logical) for logical,slot in enumerate(layout['ethernet'])}
role_by_slot.update({slot:('power',logical) for logical,slot in enumerate(layout['power'])})
slot_assignments=[]
for slot in range(4):
    role,logical=role_by_slot[slot]
    slot_assignments.append({'slot':slot,'center_yz_mm':centers[slot].tolist(),
                             'role':role,'logicalPair':logical+1})
...
wires=[]
for slot in range(4):
    role,logical_pair=role_by_slot[slot]
    for leg in range(2):
        x=np.linspace(-half,half,int(np.ceil(L/1.0))+1)
        phase=(x+half)/args.pitch*2*np.pi+leg*np.pi
        points=np.column_stack([x,centers[slot,0]+ins_r*np.sin(phase),
                                 H+centers[slot,1]+ins_r*np.cos(phase)])
        if role=='ethernet':
            group=f'signal_{logical_pair}_{leg}'
            pair=logical_pair
        else:
            pair=2+logical_pair
            if args.wiring=='same':
                group='plus' if logical_pair==0 else 'zero'
            else:
                plus_leg=0 if args.power_polarity=='aligned' or logical_pair==0 else 1
                group='plus' if leg==plus_leg else 'zero'
        terminals=[]
        leads=[]
        for side,endpoint in [(-1,points[0]),(1,points[-1])]:
            xf=side*(half+20)
            if role=='ethernet':
                terminal=np.array([xf,-4 if logical_pair==0 else 4,H+(.8 if leg==0 else -.8)])
                way=np.array([endpoint,
                    [side*(half+4),endpoint[1],endpoint[2]],
                    [side*(half+12),terminal[1],terminal[2]],terminal])
            else:
                terminal=np.array([xf,-2.4 if group=='plus' else 2.4,H+4])
                bus_y=(-6 if group=='plus' else 6)+(-.4 if logical_pair==0 else .4)
                if args.topology=='adjacent':
                    # Move the power column farther out while Ethernet fans
                    # toward its terminals; distinct lift lanes keep the
                    # corrected grid from joining unrelated conductors.
                    fan_y=4.4+1.4*(2*logical_pair+leg)
                    lane_z=H+5+1.2*(2*logical_pair+(1-leg))
                    way=np.array([endpoint,
                        [side*(half+4),fan_y,endpoint[2]],
                        [side*(half+6),fan_y,lane_z],
                        [side*(half+14),bus_y,lane_z],
                        [side*(half+18),terminal[1],lane_z],terminal])
                else:
                    # Diagonal roles first move toward central lanes. The
                    # lower and upper power pairs then leave on opposite z
                    # sides so their lifts cannot cross each other.
                    fan_y=-.6 if leg==0 else .6
                    lane_z=(H-5 if logical_pair==0 else H+5)+1.2*(1-leg)
                    fanout_x=half+5+3*logical_pair
                    way=np.array([endpoint,
                        [side*(half+4),endpoint[1],endpoint[2]],
                        [side*fanout_x,fan_y,endpoint[2]],
                        [side*(fanout_x+2),fan_y,lane_z],
                        [side*(half+14),bus_y,lane_z],
                        [side*(half+18),terminal[1],lane_z],terminal])
            leads.append(way)
        ...
        wires.append({'pair':pair,'slot':slot,'role':role,'leg':leg,'group':group,'points':full.tolist(),
...
meta={'test':'01','solver':'openEMS','solverRun':False,'wiring':args.wiring,
    'topology':args.topology,'powerPolarity':args.power_polarity,
    ...
    'fixture':'Standardized 20 mm topology-screening fanout each end; all wires first extend 4 mm axially and power wires use separated elevated lanes. Ports are test loads, not actual M12/RJ45/PHY.',
    ...
    'assessmentPurpose':'Screening: rank the tested 120 mm four-pair topology candidates by worst observed Ethernet differential pickup while retaining common-mode and combined-voltage evidence.',
    ...
    'assumptions':['Equal pair pitches and fixed relative phases/centers; not measured cable lay.',
      ...
      'Role placement changes the 20 mm fanouts with the topology; this is a coupon-plus-fixture comparison, not a de-embedded cable-only matrix.',
...
print('INPUT',json.dumps({k:meta[k] for k in ['wiring','topology','powerPolarity','length_mm','meshLines','cells','minCell_mm']}),flush=True)
...
result={'test':'01','solverRun':True,'wiring':args.wiring,'topology':args.topology,
    'powerPolarity':args.power_polarity,'elapsed_s':elapsed,'input':meta,
