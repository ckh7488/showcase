...
(async()=>{
  ...
  function renderComputed(){
    ...
    $('selected-badge').textContent=item?(item.source==='historical'?'기존 연결부 결과':'새 토폴로지 결과'):'계산 대기';
  ...
  function renderRanking(){
    ...
    $('ranking-note').textContent=results.topologyScreeningComplete?`상위 두 후보의 세분화 확인 완료 · 선택 후보: ${winner.label}`:`기본 격자 잠정 1위: ${winner.label} · 상위 두 후보 세분화 중`;
    ...
    $('ranking').innerHTML=`<table><thead><tr><th>순위</th><th>+/0 후보</th><th>최악 차동 mV/V</th><th>최악 전체 크기 mV/V</th><th>최악 공통모드 mV/V</th></tr></thead><tbody>${rows}</tbody></table><p class="muted">현재 +/+ · 0/0 배선은 후보 순위에서 제외하고 기준값으로 비교합니다.</p>`;
  }
  ...
  function renderProgress(){
    ...
    $('status-detail').textContent=stage==='complete'?`선택 후보: ${progress.winner?.label||progress.winner?.id}`:stage==='needs_review'?'저장된 계산을 확인해야 다음 단계로 갈 수 있습니다.':progress?.current?`${definition(progress.current)?.label||progress.current} 계산 중`:'기존 결과를 보존하고 별도 토폴로지 묶음으로 계산합니다.';
  ...
  function render(){renderCandidates();renderCrossSection();renderComputed();renderRanking();renderProgress();loadModel();}
