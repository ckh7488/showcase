...
const steps = [
  ...
  {kicker:"1 / 지그의 기본 상태",title:"헤드를 빼고, 지그만 측정",purpose:"새 PCB 지그 자체가 신호를 얼마나 반사하고 통과시키는지 확인합니다. 아직 프로브 감도를 측정하는 단계가 아닙니다.",actions:["RF OFF에서 P1 케이블을 지그 J1 · IN에 연결합니다.","P2 케이블을 지그 J2 · OUT에 연결합니다. 프로브는 분리하고 외장 50 Ω 부하는 붙이지 않습니다.","RF를 켜고 반사·손실·위상을 저장합니다. 케이블 위치와 지그 사진도 남깁니다."],p2:"지그 J2 · OUT",load:"사용하지 않음 — P2가 일차 종단",file:"fixture_A02_empty.s2p",key:"P2가 이미 50 Ω 수신기입니다. 지그 J2 · OUT에 부하를 추가로 병렬 연결하지 않습니다.",caption:"신호는 P1 → PCB 지그 → P2로 갑니다. 지그 출구의 P2가 50 Ω 종단 역할을 합니다.",alt:"헤드 없이 P1에서 PCB 지그 J1 · IN을 거쳐 OUT과 P2로 연결하고 외장 부하는 사용하지 않는 연결도"},
  {kicker:"2 / 프로브가 전류 경로에 주는 영향",title:"헤드를 달고, 출력을 50 Ω로",purpose:"프로브를 장착하면 원래 전류 경로가 얼마나 바뀌는지 확인합니다. 이전 단계와 같은 지그 through 연결을 유지합니다.",actions:["RF OFF에서 집게를 열어 중앙 SIG 다리만 코어 구멍으로 통과시킵니다. 양옆 귀환 레일은 밖에 둡니다.","헤드를 닫고 프로브 SMA 출력에 외장 50 Ω 부하를 연결합니다. P2는 지그 J2 · OUT에 그대로 둡니다.","RF를 켜고 through를 다시 저장해 1단계와 비교합니다. 헤드 방향·도체 위치·잠금 상태를 기록합니다."],p2:"지그 J2 · OUT — 이전 단계와 같음",load:"프로브 SMA 출력",file:"fixture_A02_probe_loaded.s2p",key:"이 단계의 S21은 지그의 전달 특성입니다. 프로브의 출력 감도 ZT가 아닙니다.",caption:"P1 → 지그 → P2는 그대로입니다. 새로 붙인 프로브의 출력만 외장 50 Ω로 종단합니다.",alt:"P2는 지그 J2 · OUT에 유지하고 중앙 경로에 장착한 프로브 출력에 외장 50옴 부하를 연결하는 그림"},
  {kicker:"3 / 프로브가 내보내는 신호",title:"P2와 외장 부하의 자리를 바꿉니다",purpose:"이번에는 지그 입력에 대해 프로브가 내보내는 신호를 읽습니다. 전류 모델을 입증하기 전에는 상대 S21로 기록합니다.",actions:["RF OFF. P2 케이블을 지그 J2 · OUT에서 빼고, 외장 50 Ω 부하를 프로브 출력에서 뺍니다.","외장 50 Ω 부하는 지그 J2 · OUT으로 옮기고, P2 케이블은 프로브 SMA 출력에 연결합니다. P1은 그대로 둡니다.","두 경로의 종단을 확인한 뒤 RF를 켜고 S-parameter와 메타데이터를 저장합니다."],p2:"프로브 SMA 출력",load:"지그 J2 · OUT",file:"fixture_A02_probe_transfer.s2p + .json",key:"지그 J2 · OUT은 외장 부하로, 프로브 출력은 P2로 각각 한 번씩 종단됩니다.",caption:"P2가 프로브로 이동합니다. 프로브에 있던 외장 부하는 지그 출구로 이동합니다. 두 SMA 위치를 모델에서 비교해 보세요.",alt:"P1은 지그 J1 · IN에 유지하고 지그 J2 · OUT에는 외장 50옴 부하, 프로브 출력에는 P2 케이블을 연결한 전달 측정 그림"}
];
let activeStep=0;
function selectStep(index){
  ...
  document.querySelector("#calibration-cad .cad-badge").textContent=index===0?"교정 표준·케이블 · 외형 예시":"현재 · 평판 PCB A0.2 / 집게 A0.3";
...
const pathNotes={
 ["A02"]:{all:"같은 한 장의 PCB 안에서 중앙 SIG와 바깥 GND를 분리합니다. 두 창 사이 중앙 다리만 코어 안으로 들어갑니다.",signal:"① 신호: J1 중심 핀 → 같은 보드의 SIG 동박 → 중앙 다리 → J2 중심 핀. 양면 모두 SIG이며 점퍼선이 없습니다.",return:"② 귀환: J2 외피의 GND 핀 → 같은 보드의 양옆 바깥 레일 → J1 외피의 GND 핀. 코어 구멍 밖으로 돌아갑니다."},
 A01:{all:"이전 비교용 구조입니다. Frame과 양끝 EndPanel 2장을 접합합니다. 아래 조립·교정 설명은 현재 A0.2 기준입니다.",signal:"이전 A0.1: SMA 중심 → 점퍼 → Frame 중앙 SIG → 점퍼 → 반대 SMA 중심입니다.",return:"이전 A0.1: SMA 외피 → EndPanel → Frame 양옆 귀환 → 반대 EndPanel → 반대 SMA 외피입니다."}
};
window.updateRFCPPath=()=>{const v=window.rfcpViewers?.structure;const mode=document.querySelector('button[data-path][aria-pressed="true"]').dataset.path;document.getElementById("path-explanation").textContent=pathNotes[v?.revision||"A02"][mode];};
document.querySelectorAll("button[data-path]").forEach(btn=>btn.addEventListener("click",()=>{const mode=btn.dataset.path;document.querySelectorAll("button[data-path]").forEach(b=>b.setAttribute("aria-pressed",String(b===btn)));window.rfcpViewers?.structure?.setPath(mode);window.updateRFCPPath();}));
...
if(document.getElementById("s21-db"))updateExample();

const requestedStep=Number(new URLSearchParams(location.search).get("step")||0);
if(document.getElementById("calibration-cad"))selectStep(Number.isInteger(requestedStep)&&requestedStep>=0&&requestedStep<=3?requestedStep:0);
