...
- **폴더 안에 완결:** 다른 보고서의 라이브러리나 로컬 작업 폴더에 의존하지 않습니다. 사용하는 버전의 라이브러리와 라이선스를 함께 보관합니다. 임의 CDN 최신 버전으로 바꾸지 않습니다.
...
> 이 저장소의 AGENTS.md와 README.md를 읽고, 완성된 HTML 보고서를 reports/새이름/에 필요한 자산과 함께 추가해 줘. 원래 인터랙션을 유지하고 reports.json에 카드 한 개를 등록해. 기존 보고서와 랜딩페이지는 유지하고, 로컬 검증과 브라우저 테스트 후 배포해 줘. 배포 완료와 실제 URL 동작까지 확인해 줘.
