...
화면 이름은 **ATLAS**, 부제는 **Interactive Library**, 목록 제목은 **Collections**로 통일합니다. 저장소·공개 URL의 `showcase`와 기존 `showcase-update` 배포 명령은 그대로 사용합니다.
...
"collections": [
  {
    "id": "emc",
    "title": "EMC",
    "summary": "구조 이해부터 설계 판단까지 이어지는 학습 자료"
  }
]
```

보고서 항목에 `"collection": "emc"`를 추가하면 해당 섹션에 표시됩니다. 섹션 순서는 `collections`, 섹션 안의 문서 순서는 `reports` 배열을 따릅니다. `collection`이 없는 기존 보고서는 “설계 · 실험 · 분석”에 표시됩니다. `other`는 이 기본 모음에 예약된 이름입니다. 이후 EMC 자료도 새 폴더와 목록 항목만 추가하면 됩니다.
...
- **폴더 안에 완결:** 공통 스타일 `../../assets/theme.v1.css`와 ATLAS 파비콘 `../../assets/favicon.svg`를 제외하면 다른 보고서나 로컬 작업 폴더에 의존하지 않습니다. 사용하는 버전의 라이브러리와 라이선스를 함께 보관합니다. 임의 CDN 최신 버전으로 바꾸지 않습니다.
- **상대 경로:** `./viewer.js`, `assets/model.glb`처럼 씁니다. `/assets/...`는 GitHub Pages의 `/showcase/` 경로를 벗어나므로 피합니다. 파일명 대소문자도 정확히 맞춥니다.
- **목록 복귀:** 보고서 상단에 `<a href="../../" aria-label="ATLAS 라이브러리로 돌아가기">← ATLAS</a>`를 둡니다. 새 탭을 강제하지 않아 브라우저 뒤로 가기도 자연스럽게 동작하게 합니다.
