...
Keep the first 8 ns diagnostic records unchanged. Recompute only DFTs of saved
...
def response_at(folder, original, stop_ns):
    ...
    sc = ref/inc[2]
    arrays = dict(frequency_Hz=freq, Vin=vin, Vdiff=v[3:5], Vcommon=cm,
                  Hdiff=hd, Hcommon=hc, HdiffNear=v[:2]/vin, HcommonNear=cm_near/vin,
                  portVoltage=v, portCurrent=i, Scolumn=sc)
    record = {k: encode(arrays[k]) for k in OUTPUTS+['Vin']}
...
for name in ['run_cable_screening.py', 'publish_screening.py']:
...
for wiring in ['same', 'mixed']:
    for level in ['coarse', 'fine']:
        ...
        r['termination'] = {'lastReportedEnergy_dB': final,
            ...
            'reason': 'energy_criterion' if final <= -49.95 else 'predeclared_16ns_observation_limit',
        ...
        files = ['input.json', 'model.json', 'mesh.json', 'geometry.xml', 'openEMS.xml']
...
for wiring in ['same', 'mixed']:
    ...
    checks[f'{wiring}CoarseMeshUnchangedFromFirstRun'] = read(runs/f'test01-{wiring}-coarse/mesh.json') == read(initial/f'test01-{wiring}-coarse/mesh.json')
...
for r in records:
    assessment['timeComparisons'][r['id']] = {key: {
        ...
        for a,b in [(8,12),(12,15.5),(8,15.5)]} for key in OUTPUTS}
...
manifest = {'revision': 'screening-openems-01', 'solverRun': True,
