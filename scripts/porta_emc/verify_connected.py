...
for r in manifest['references']+manifest['cableRuns']:
    ...
    for ns, w in r['windows'].items():
        ...
        for k in [0,90,190]:
            ...
            np.testing.assert_allclose(transform('port_ut_6')/vin,npz['Hpower'][k],rtol=1e-10,atol=1e-13)
            for pair in [0,1]:
...
print(json.dumps({'passed':True,'evidenceHashes':hashes,'independentlyCheckedProbeOutputs':points,'oldInvalidMeshUsedAsBaseline':False}))
