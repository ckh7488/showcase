...
**Current continuation entry point:** [Portable archive and handoff](../../simulations/porta-openems/README.md).
The connected TEST 01 replacement comparison and TEST 02 time/transverse-grid review are now
available. The original-run and Review 06 descriptions below are historical;
use the handoff and each report's explicit verdict for current status.
...
**Review 06 supersedes the physical interpretation of the first TEST 01
numbers.** Its 0.8 mm axial mesh disconnects the power fanout wires, despite
connected CAD centerlines. The four Ethernet wires remain connected. Native
openEMS PEC dumps of the near fanout independently confirm this difference
against the 0.4 mm axial / 0.16 mm transverse mesh. Whole-model graph audits
cover both wiring assignments. This is a topology failure, not a small
percentage accuracy disagreement. No replacement transient comparison is
complete; the attempted longer runs were deliberately stopped for this review.

`publish_connection_review.py` and `verify_connection_review.py` package and
check the actual evidence in `records/screening-review-06/`. The maintained
`run_cable_screening.py` now defaults to 0.4 mm axial spacing and audits the
intended six electrical nets before native setup. `--geometry-only` performs
that audit without a transient. A connected mesh is necessary but still needs
power-through sanity and the intended time/grid output comparison. Original
scripts and first-run records are retained for historical reproducibility.

The following describes the initially planned follow-up, which was superseded
by the connection review before its four-run time/grid comparison completed.

The user clarified that this stage should identify consequential coupling and
prioritize measurements or design changes. It is not a 1% accuracy exercise.
`run_cable_screening.py` retains the first runner's physical geometry, excitation,
materials, ports and boundary conditions, adding a declared 16 ns maximum
physical duration. The original 1e-5 energy target remains recorded separately.

The follow-up compares the original 0.2 mm transverse / 0.8 mm nominal axial
grid with a 0.16 mm / 0.4 mm grid. `publish_screening.py` requires all four real
responses before packaging. It selects actual samples up to 8, 12 and 15.5 ns;
15.5 ns is covered by both meshes' probe sampling before the 16 ns run limit.
It neither resamples the traces nor extends an unrecorded tail. Both Ethernet
pairs at both ends can be evaluated from already-recorded ports and voltage
paths without an additional excitation.

The comparison retains absolute changes, peak output and frequency, and
prominent-band magnitude changes. A 3 dB change is an attention guide; a factor
of two describes a clear wiring effect or dominant pair. These guides are not
automatic product pass/fail criteria or certified error bounds. The written
assessment must state which locations and effects remain stable and which do
not. First-run records remain untouched in `records/initial-openems/`.

`verify_screening.py` checks source and evidence hashes and independently sums
raw probe samples at selected frequencies. It also checks that rerunning the
unchanged coarse model reproduces its original first 8 ns response.
