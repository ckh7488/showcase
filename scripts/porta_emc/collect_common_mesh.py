...
manifest={'revision':a.out.name,'test':'02','expectedCases':names,'cases':cases,'updatedAtUTC':datetime.now(timezone.utc).isoformat(),'scientificVerdictComplete':False,'status':'all_case_data_checked_review_pending' if len(cases)==4 else 'partial_mesh_data_checked','comparisons':comparisons}
